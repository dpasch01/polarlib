...
def G_to_fi(G):
    ...
    neg_edgelist = [(node_to_id[e[0]], node_to_id[e[1]], {'weight': -1}) for e in list(edgelist) if
                    e[2]['weight'] < 0.0]
...
def calculate_frustration_index(sign_G, sign_matrix, sign_edgelist):
    ...
    for triangle in graph_triangles:
        ...
        model.addConstr(x[j] + z[(i, k)] >= z[(i, j)] + z[(j, k)],
                        'triangle1' + ',' + str(i) + ',' + str(j) + ',' + str(k))
        model.addConstr(x[i] + z[(j, k)] >= z[(i, j)] + z[(i, k)],
                        'triangle2' + ',' + str(i) + ',' + str(j) + ',' + str(k))
        model.addConstr(x[k] + z[(i, j)] >= z[(i, k)] + z[(j, k)],
                        'triangle3' + ',' + str(i) + ',' + str(j) + ',' + str(k))
        model.addConstr(1 + z[(i, j)] + z[(i, k)] + z[(j, k)] >= x[i] + x[j] + x[k],
    ...
    for v in model.getVars():
        if 'x' in v.varName:
            optimal_solution['x'][v.varName[1:]] = v.x
        elif 'z' in v.varName:
            optimal_solution['z'][v.varName[1:]] = v.x
...
def triadic_balance(G, triad):
    ...
    for c in itertools.combinations(triad, 2):
        if G.get_edge_data(c[0], c[1])['weight'] < 0.0:
            minus_str += '-'
        else:
            plus_str += '+'
