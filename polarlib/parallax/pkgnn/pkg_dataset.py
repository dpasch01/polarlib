import networkx as nx, os, numpy, pandas as pd, torch, pickle
...
class PKGDataset(Dataset):
    ...
    @staticmethod
    def convert_to_dataframe(micro_pkg_path, output_dir):

        df = []

        for root, folders, files in os.walk(micro_pkg_path):

            for f in files:

                f = os.path.join(root, f)

                if not f.endswith('micro.pckl'): continue
                ...
                for e in micro_pkg.edges(data=True):
        ...
        os.makedirs(os.path.join(output_dir, 'parallax/dataset/raw'), exist_ok=True)

        df.to_csv(os.path.join(output_dir, 'parallax/dataset/raw/micro.csv'), index=None)
